...
def search_legal_database(query: str) -> str:
    ...
    for entry in LEGAL_KNOWLEDGE:
        overlap = sum(1 for kw in entry["keywords"] if kw in query_lower or query_lower in kw)
        if overlap > 0:
...
QUESTION = "Công ty đơn phương chấm dứt hợp đồng lao động do nhân viên ốm đau 10 tháng chưa khỏi thì có đúng luật không? Và thời hiệu khởi kiện (statute of limitations) cho vụ án hợp đồng (contract) là bao lâu?"
